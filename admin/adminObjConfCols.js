...
{
	document.getElementById('linkObjectRS').style.display = t == 'R' ? '' : 'none';
	document.getElementById('linkObjectForm').style.display = t == 'F' ? '' : 'none';
	
	changeObject(t == 'R' ? document.getElementById('linkObjectRS') : document.getElementById('linkObjectForm'));
}
...
function changeObject(obj)
{
	...
	switch (LinkType)
	{
		case 'R':
			...
			document.getElementById('linkLinkData').style.display = 'none';
			break;
		case 'F':
			if (obj != '')
			{
				document.getElementById('linkLinkData').style.display = '';
			}
			else
			{
				document.getElementById('linkLinkData').style.display = 'none';
...
function setRSVars(rsVars)
{
	...
	else
	{
		...
		for (var i = 0;i<rsVars.length;i++)
		{
			...
			newCell = newRow.insertCell(newRow.cells.length);
			var strCell = '<font color="#4783C5" face="Verdana" size="1">' +
								...
								'<option></option>' +
								'<option value="@' + (typeID == 'A' ? 'ID' : 'LogNum') + '">' + (typeID == 'A' ? LtxtActionID : DtxtLogNum) + '</option>' +
								'</select></font>' +
