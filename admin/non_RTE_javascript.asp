				...
				if ((insertText != null) && (insertText != '') && (code == 'URL')){
					insertCode = prompt('<% = strTxtEnterHeperlinkURL %>', 'http://');
						...
						if ((insertCode != null) && (insertCode != '') && (insertCode != 'http://')){					
							document.getElementById(textArea).value += '<a href="RTE/'%20+%20insertCode%20+%20'">' + insertText + '</a>';
						}
				}
		}
		else {
	...
	if ((code != '') && (code == 'IMG')){
		if (promptOn == true){	
			insertCode = prompt('<% = strTxtEnterImageURL %>', 'http://');
				...
				if ((insertCode != null) && (insertCode != '')){					
				document.getElementById(textArea).value += '<img src="RTE/'%20+%20insertCode%20+%20'" />';
			}			
		}
		else {
