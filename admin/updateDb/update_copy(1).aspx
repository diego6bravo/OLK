...
<!--|P:LangLink|-->
<script runat="server">
Dim olkip, olklogin, olkpass, BackupPath, dbName, licip, licport as String
Dim dbID as Integer
Dim Err as Boolean
...
Function UpdateDB() As Boolean
    ...
    Select Case LawsSet
    	Case "CL", "CR", "GT", "US", "CA"
    		LawsSet = "MX"
    	Case "AT", "AU", "BE", "CH", "CZ", "DE", "DK", "ES", "FI", "FR", "HU", "IT", "NL", "NO", "PL", "PT", "RU", "SE", "SK", "GB", "ZA"
    		LawsSet = "PA"
	End Select
            ...
            If dr(1) = "Y" Then
                ...
                For Each dr2 In ds.Tables("const").Rows
                   ...
                   ' Try
                        sqlCm.ExecuteNonQuery()
                   ' Catch ex As Exception
                   '     ErrMsg &= "Failed to drop contraint " & dr2(1) & " from TMP" & dr(0) & ":" & ex.Message
                   '     Err = True
                   '     return false
                   ' End Try
                Next
    ...
    For Each drv In dv
        Select Case drv("Type")
            Case "P"
                sqlStr = sqlStr & "if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[DB{1}{0}]') and OBJECTPROPERTY(id, N'IsProcedure') = 1) " & _
                "drop procedure [dbo].[DB{1}{0}] "
            Case "F"
                sqlStr = sqlStr & "if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[DB{1}{0}]') and xtype in (N'FN', N'IF', N'TF')) " & _
                "drop function [dbo].[DB{1}{0}] "
        End Select
    Next
    sqlCn.ChangeDatabase("OLKCommon")
    sqlCm = New SqlCommand(string.Format(sqlStr, dbID, drv("TableID")), sqlCn, sqlTran)
    sqlCm.ExecuteNonQuery()
