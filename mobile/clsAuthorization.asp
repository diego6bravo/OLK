...
Class clsAuthorization
	...
	Public Function HasAuthorization(ByVal AutID)
	...
	Private Property Get GetObjProperty(ByVal autString, ByVal PropType) 
		Select Case PropType 
			Case "C" 'Document Confirm Process 
				GetObjProperty = InStr(autString, "{C}") <> 0 
 ...
 End Property 
	...
	Public Property Get HasObjConfirmAccess 
		HasObjConfirmAccess = Session("useraccess") = "P" or _ 
								InStr(Session("aut_77"), "{C}") <> 0 or _ 
								InStr(Session("aut_45"), "{C}") <> 0 or _ 
								InStr(Session("aut_78"), "{C}") <> 0 or _ 
								InStr(Session("aut_27"), "{C}") <> 0 or _ 
								InStr(Session("aut_30"), "{C}") <> 0 or _ 
								InStr(Session("aut_31"), "{C}") <> 0 or _ 
								InStr(Session("aut_29"), "{C}") <> 0 or _ 
								InStr(Session("aut_34"), "{C}") <> 0 or _ 
								InStr(Session("aut_35"), "{C}") <> 0 
	End Property 
 ...
 Sub LoadAuthorization(ByVal SlpCode, ByVal dbName) 
     ...
     Else 
     sql = "select [Authorization] from [" & dbName & "]..OLKAgentsAccess where SlpCode = " & SlpCode
     End If 
     ...
     If Not IsNull(rsAccess(0)) Then 
         ...
         For i = 0 to UBound(arrAut) 
             ...
             Select Case autType
                 Case "S"
					Session("aut_" & Replace(autType, "S", "")) = arrAut(i) 
                 Case "R"
