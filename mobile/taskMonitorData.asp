	...
	Select Case Task
		Case 1
			...
			sql = sql & "left outer join R3_ObsCommon..TLOG T5 on T5.LogNum = T0.LogNum " & _
						"where T0.Status in ('O', 'P', 'E') and Left(T0.ExecAt, 1) = 'O' and Left(T0.ExecAt, 2) <> 'OP' "
		...
		Case 12
			...
			If myAut.HasAuthorization(155) Then strObjCode = myAut.ConcValue(strObjCode, "22")
			If myAut.HasAuthorization(183) Then strObjCode = myAut.ConcValue(strObjCode, "540000006")
		...
		Case 5
			...
			If myApp.EnableOPQT Then ObjCode = myAut.ConcValue(ObjCode, "540000006")
		...
		Case 14, 15, 16, 17, 18
			...
			cmd.ActiveConnection = connCommon
			cmd.CommandType = &H0004
			cmd.CommandText = "DBOLKGetTaskMonitorData" & Session("ID")
			cmd.Parameters.Refresh
			cmd("@SlpCode") = Session("vendid")
			cmd("@InfoType") = Task
			set rs = Server.CreateObject("ADODB.RecordSet")
			set rs = cmd.execute()
			GetTaskMonitorInfo = rs(0)
	End Select
End Function
%>